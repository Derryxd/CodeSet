...
class WindroseAxes(PolarAxes):
    ...
    def _init_plot(self, direction, var, **kwargs):
        ...
        ## !!!!! changed by zqq0ew0 at 2018.12.19 for use windenergy
        if 'we' in kwargs:
            import pandas as pd
            
            dir_bins = [0,11.25,33.75,56.25,78.75,101.25,123.75,146.25,168.75,191.25,213.75,236.25,258.75,281.25,303.75,326.25,348.75,371.25]
            bins_we = bins.tolist()
            bins_we.append(np.inf)
            n_we = len(bins)
            n_wd = len(dir_bins)-1
            we_table = np.zeros([n_we,n_wd])
            for i in range(n_we):
                we = var.copy()
                we = we[we>=bins_we[i]]
                we = we[we<bins_we[i+1]]
                for j in range(n_wd):
                    wd = direction.copy()
                    wd = wd[wd>=dir_bins[j]]
                    wd = wd[wd<dir_bins[j+1]]
                    we_wd = pd.concat([wd,we],axis=1,join='inner')
                    we_table[i,j] = we_wd.iloc[:,1].sum()
                    
            we_table[:, 0] = we_table[:, 0] + we_table[:, -1]
            # and remove the last col
            we_table = we_table[:, :-1]
            if normed:
                we_table = we_table * 100 / we_table.sum()
            
            del kwargs['we']
            self._info['table'] = we_table
